...
from LongLifeMethod.FPKD import Appr,LongLifeTrain
...
if __name__ == '__main__':
    ...
    net_glob = RepTailResNet(output=100,nc_per_task=10)
    net_glob.train()
    if args.load_fed != 'n':
    ...
    if args.alg == 'fedrep' or args.alg == 'fedper':
        if 'cifar' in args.dataset or 'miniimagenet' in args.dataset or 'FC100' in args.dataset or 'Corn50' in args.dataset:
    ...
    start = time.time()
    task=-1
    apprs = [Appr(copy.deepcopy(net_glob),PackNet(args.task),copy.deepcopy(net_glob), None,lr=args.lr, nepochs=args.local_ep, args=args) for i in range(args.num_users)]
    print(args.round)
    for iter in range(args.epochs):
        ...
        if iter % args.round == args.round-1:
            ...
            model_save_path = './save/FPKD/0.4/accs_FPKD_lambda_'+str(args.lamb) +str('_') + args.alg + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
    ...
    print(times)
    print(accs)
    base_dir = './save/FPKD/0.4/accs_FPKD_lambda_'+str(args.lamb) +str('_') + args.alg + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
