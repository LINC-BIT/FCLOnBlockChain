...
class Appr(object):
    ...
    def __init__(self, model, tr_dataloader,nepochs=100, lr=0.001, lr_min=1e-6, lr_factor=3, lr_patience=5, clipgrad=100,
        ...
        self.alpha = args.AMPalpha
        self.lamb = args.AMPlamb
    ...
    def set_model(self,model):
        self.model = model
    def set_umodel(self,umodel):
        self.umodel = deepcopy(self.model)
        self.umodel.load_state_dict(umodel)
        self.umodel.cuda()
    ...
    def train(self, t):
        ...
        # Fisher ops
        if self.umodel is not None:
            for para, upara in zip(self.model.parameters(), self.umodel.parameters()):
                temp = ((para - upara).pow(2)) * (self.lamb / (2 * self.alpha)) + para
                para.data = temp.clone()
