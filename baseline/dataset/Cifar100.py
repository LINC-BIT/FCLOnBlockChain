...
class testmodel(torch.nn.Module):
    def __init__(self):
        ...
        self.net_glob.fc = torch.nn.Linear(self.net_glob.fc.in_features,100)
    def forward(self,x):
        out = self.net_glob(x)
        return out

def main():
    task = Cifar100Task('../data/cifar100',task_num=1)
    ...
    net_glob = testmodel()
    net_glob.cuda()
    opt = torch.optim.Adam(net_glob.parameters(), 0.0001)
