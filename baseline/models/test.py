...
def test_img_local(net_g, dataset, args,t,idx=None,indd=None, user_idx=-1, idxs=None,appr = None,num_classes=10,glob_classify =None):
...
def test_img_local_meta(net_g, dataset, args, t, idx=None, indd=None, user_idx=-1, idxs=None, appr=None,num_classes = 10):
    ...
    if 'femnist' in args.dataset:
        ...
        usr = idx
        for j in range(len(dataset[usr]['x'])):
            datatest_new.append((torch.reshape(torch.tensor(dataset[idx]['x'][j]), (1, 28, 28)),
                                 torch.tensor(dataset[idx]['y'][j])))
    elif 'sent140' in args.dataset:
        leaf = True
        datatest_new = []
        for j in range(len(dataset[idx]['x'])):
    ...
    if leaf:
        data_loader = DataLoader(DatasetSplit_leaf(datatest_new, np.ones(len(datatest_new))),
    ...
    for idx, (data, target) in enumerate(data_loader):
        if idx == 0:
            ...
            if 'sent140' in args.dataset:
                ...
                data, targets = torch.from_numpy(input_data).to(args.device), torch.from_numpy(target_data).to(
                    args.device)
                net_g.zero_grad()
    ...
    accuracy = 100.00 * float(correct) / count
    return accuracy, test_loss
def test_img_local_all(net, args, dataset_test, dict_users_test,t,w_locals=None,w_glob_keys=None, indd=None,dataset_train=None,dict_users_train=None, return_all=False,write =None,apprs = None,meta=False,num_classes = 10,glob_classify=None):
        ...
        for u in range(t+1):
            ...
            if 'femnist' in args.dataset or 'sent140' in args.dataset:
                ...
                if meta:
                    ...
                    a, b = test_img_local(net_local, dataset_test[u], args,u, user_idx=idx, idxs=dict_users_test[idx],appr = appr,num_classes=num_classes,glob_classify = glob_classify)
                all_task_acc+=a
                all_task_loss+=b
        all_task_acc /= (t+1)
        all_task_loss /= (t+1)
