...
class PackNet():
    ...
    def __init__(self, n_tasks, prune_instructions=.9, prunable_types=(nn.Conv2d, nn.Linear)):
    ...
    def prune(self, t,model, prune_quantile):
        ...
        """
        # Calculate Quantil
        all_prunable = torch.tensor([]).to(device)
        for name, param_layer in model.named_parameters():
            if 'bias' not in name:

                # get fixed weights for this layer
                prev_mask = torch.zeros(param_layer.size(), dtype=torch.bool, requires_grad=False).to(device)
        ...
        with torch.no_grad():
            for name, param_layer in model.named_parameters():
                if 'bias' not in name:
                    ...
                    prev_mask = torch.zeros(param_layer.size(), dtype=torch.bool, requires_grad=False).to(device) # p
                    for task in self.masks:
                        if name in task:
                            prev_mask |= task[name]
