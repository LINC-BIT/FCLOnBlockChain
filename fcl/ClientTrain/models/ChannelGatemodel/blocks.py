...
from ClientTrain.models.ChannelGatemodel.buffer_container import BufferList
from ClientTrain.models.ChannelGatemodel.Freezable_layers import FreezableInstanceNorm2d, FreezableBatchNorm2d, FreezableConv2d
...
class GatedConvResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_opt_path:
            ...
            after_bn_opt = []
            for task_input, task_fbn_opt in zip(opt_out.reshape(bs, N_tasks, N_ch, H, W).transpose(0, 1),
                                                self.fbns_opt):
...
class BaseGatedConv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, freezing_method, aggregate_firing=False, N_tasks=1, conv_params: dict = None):
        super().__init__()
        self.N_tasks = N_tasks
        self.in_ch = in_ch
        self.out_ch = out_ch
