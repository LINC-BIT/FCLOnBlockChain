...
from Agg.OTFusion.Fusion_hyperparameters import SixCNN as SixParameter
from Agg.OTFusion.utils import dotdict, get_timestamp_other, mkdir
import json
import copy
import os
import Agg.OTFusion.utils as utils
import Agg.OTFusion.Fusion_hyperparameters.ResNet18 as Res18Parameter
import Agg.OTFusion.Fusion_hyperparameters.TextCNN as TextCNNParameter
import Agg.OTFusion.Fusion_hyperparameters.GCN as GCNParameter
# commenting this now
...
def get_parser():
    ...
    parser.add_argument('--fusion-model',default='ResNet18',type=str,help='model of fusion ')
    parser.add_argument('--clmethod', default='EWC', type=str, help='CL of method')
    ...
    parser.add_argument('--dataset', default='mnist', type=str, choices=['mnist', 'Cifar10', 'cifar100', 'miniimagenet'],
                        help='dataset to use for the task')
    parser.add_argument('--num-models', default=2, type=int, help='number of models to ensemble')
    parser.add_argument('--model-name', type=str, default='mlpnet',
    ...
    parser.add_argument('--server-gpu', default=0, type=int, help='GPU id to use')
    ...
    parser.add_argument('--round', type=int, default=1, help='train number of the per-task')
    parser.add_argument('--model', type=str, default='cnn', help='model name')
    parser.add_argument('--kd_epoch', default=5, type=int, help='kd training epoch')

    # bcfl arguments
    parser.add_argument('--malicious_clients', type=int, nargs='*', default=[], help='An list of malicious clients')
    parser.add_argument('--victim', default=0, type=int, help='victim index')
    parser.add_argument('--committee_num', type=int, default=3, help='num of committee members')
    parser.add_argument('--defend', action='store_true', help='abandon clients with potential issues or not')
    
    parser.set_defaults()
    if parser.parse_args().fusion_model =='ResNet18':
        parser = Res18Parameter.set_fusion_hyperparameters(parser)
    elif parser.parse_args().fusion_model =='sixcnn':
        parser = SixParameter.set_fusion_hyperparameters(parser)
    elif parser.parse_args().fusion_model =='textcnn':
        parser = TextCNNParameter.set_fusion_hyperparameters(parser)
    elif parser.parse_args().fusion_model =='gcn':
        parser = GCNParameter.set_fusion_hyperparameters(parser)
