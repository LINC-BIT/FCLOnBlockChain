...
from ClientTrain.LongLifeMethod.FedKNOW import LongLifeTrain as FedKNOW_LongLifeTrain
...
if __name__ == '__main__':
    ...
    write = SummaryWriter('/data/lpyx/FedAgg/ClientTrain/log/multi/'+args.clmethod+'/'+args.clmethod+'localall_server_epoch10_high20_dag_' + args.dataset+'_'+'round' + str(args.round) + '_frac' + str(args.frac))
    # build model
    # net_glob = get_model(args)
    multi_model_dict = get_model_dict()
    net_glob = SixCNN([3,32,32],outputsize=200)
    net_glob.train()
    ...
    start = time.time()
    task=-1
    kd_model = KDmodel([3,32,32],200).to(args.device)
    apprs =[]
    for idx in range(args.num_users):
        if multi_model_dict[idx]['cl_method'] == 'EWC':
            appr = EWC_Appr(multi_model_dict[idx]['model'].to(args.device), None, lr=multi_model_dict[idx]['lr'], nepochs=multi_model_dict[idx]['local_epoch'], args=args,kd_model=kd_model)
        elif multi_model_dict[idx]['cl_method'] == 'GEM':
            appr = GEM_Appr(multi_model_dict[idx]['model'].to(args.device), kd_model, 3 * 32 * 32, 100, 20, args,lr=multi_model_dict[idx]['lr'])
        elif multi_model_dict[idx]['cl_method'] == 'FedKNOW':
            appr = FedKNOW_Appr(multi_model_dict[idx]['model'].to(args.device), PackNet(args.task, device=args.device), copy.deepcopy(net_glob), None,
                          lr=multi_model_dict[idx]['lr'], nepochs=multi_model_dict[idx]['local_epoch'], args=args, kd_model=kd_model)
        elif multi_model_dict[idx]['cl_method'] == 'MAS':
            appr = MAS_Appr(multi_model_dict[idx]['model'].to(args.device), None, lr=multi_model_dict[idx]['lr'], nepochs=multi_model_dict[idx]['local_epoch'], args=args,
                          kd_model=kd_model)
        elif multi_model_dict[idx]['cl_method'] == 'Packnet':
            appr = Packnet_Appr(multi_model_dict[idx]['model'].to(args.device), None, lr=multi_model_dict[idx]['lr'], nepochs=multi_model_dict[idx]['local_epoch'], args=args,
                 kd_model=kd_model)
        elif multi_model_dict[idx]['cl_method'] == 'ChannelGate':
            appr = ChannelGate_Appr(multi_model_dict[idx]['model'].to(args.device), None, lr=multi_model_dict[idx]['lr'], nepochs=multi_model_dict[idx]['local_epoch'], args=args,
                 kd_model=kd_model)
        else:
    ...
    for iter in range(args.epochs):
        ...
        acc_test, loss_test = test_img_local_multi_all(multi_model_dict, args, dataset_test, dict_users_test,task,apprs=apprs,return_all=False,write=write,round=iter,client_task=client_task)
