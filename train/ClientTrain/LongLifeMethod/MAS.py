...
class Appr(object):
    ...
    def __init__(self, model, tr_dataloader,nepochs=100, lr=0.001, lr_min=1e-6, lr_factor=3, lr_patience=5, clipgrad=100,
                 args=None, kd_model=None, kd_epoch=0):
        self.model = model
    ...
    def train(self, t: int):
        ...
        for e in range(self.nepochs):
            # Train
            # 训练更新的是self.model
            self.train_epoch_rep(t, e, kd_lambda)
            ...
            if e % self.e_rep == self.e_rep -1:
                ...
                logger.info('| Epoch {:3d} | Train: loss={:.3f}, acc={:5.1f}% |'.format(e + 1,  train_loss, 100 * train_acc))
...
def LongLifeTrain(args, appr: Appr, aggNum, writer, idx):
    print('cur round :' + str(aggNum)+'  cur client:' + str(idx))
    logger.info('cur round :' + str(aggNum)+'  cur client:' + str(idx))
