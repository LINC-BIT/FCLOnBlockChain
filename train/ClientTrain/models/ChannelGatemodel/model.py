...
from ClientTrain.models.ChannelGatemodel.blocks import (FreezingMethod,
                        GatedConvDownBlock,
                        GatedConvSameBlock,
                        GatedConvResBlock,
                        TaskAgnosticWrapper)
...
class Backbone(nn.Module):
    ...
    def __init__(self, in_ch, conv_ch, freezing_method):
        ...
        elif cfg.ARCH == 'SixCNN':
            self.layers = self.create_SixCNN()
        else:
...
def freeze_relevant_kernels(litmodel, val_loader,
                            ...
                            save_freqs=False, save_fname='',t=0):
    """
    ...
    for x, y in iterator:
        x = x.to(cfg.DEVICE)
        task_idx = torch.full_like(y,t).to(cfg.DEVICE)
        _ = litmodel(x, task_idx)
