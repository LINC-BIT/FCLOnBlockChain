...
import torch
import numpy as np
import Agg.OTFusion.routines as routines
from Agg.OTFusion.model import get_model_from_name
import Agg.OTFusion.utils as utils
from Agg.OTFusion.ground_metric import GroundMetric
import math
...
def get_wassersteinized_layers_modularized(args, networks, activations=None, eps=1e-7, test_loader=None):
    ...
    if args.server_gpu==-1:
        ...
        device = torch.device('cuda:{}'.format(args.server_gpu))
    ...
    for idx, ((layer0_name, fc_layer0_weight), (layer1_name, fc_layer1_weight)) in \
        ...
        if args.server_gpu!=-1:
            T_var = torch.from_numpy(T).cuda(args.server_gpu).float()
        else:
        ...
        if args.correction:
            if not args.proper_marginals:
                ...
                if args.server_gpu != -1:
                    # marginals = torch.mv(T_var.t(), torch.ones(T_var.shape[0]).cuda(args.server_gpu))  # T.t().shape[1] = T.shape[0]
                    marginals = torch.ones(T_var.shape[0]).cuda(args.server_gpu) / T_var.shape[0]
                else:
...
def update_model(args, model, new_params, test=False, test_loader=None, reversed=False, idx=-1):
    ...
    if args.server_gpu != -1:
        updated_model = updated_model.cuda(args.server_gpu)
...
def _custom_sinkhorn(args, mu, nu, cpuM):
    if not args.unbalanced:
        if args.sinkhorn_type == 'normal':
            ...
            T, _ = utils.sinkhorn_loss(cpuM, mu, nu, gpu_id=args.server_gpu, epsilon=args.reg, return_tmap=True)
        else:
...
def _compute_marginals(args, T_var, device, eps=1e-7):
    if args.correction:
        if not args.proper_marginals:
            ...
            if args.server_gpu != -1:
                marginals = marginals.cuda(args.server_gpu)
...
def get_acts_wassersteinized_layers_modularized(args, networks, activations, eps=1e-7, train_loader=None, test_loader=None):
    ...
    #         enumerate(zip(network0_named_params, network1_named_params)):
        ...
        if  activations_0.shape[0] != fc_layer0_weight.shape[0]:
            activations_0 = activations_0.t()
        if  activations_1.shape[0] != fc_layer1_weight.shape[0]:
            activations_1 = activations_1.t()
...
def get_network_from_param_list(args, param_list, test_loader,output):

    # print("using independent method")
    new_network = get_model_from_name(args, idx=1,output=output)
    ...
    return 0, new_network

def geometric_ensembling_modularized(args, networks, train_loader, test_loader, activations=None,output=100):
    ...
    return get_network_from_param_list(args, avg_aligned_layers, test_loader,output=output)
