...
class FedKD():
    ...
    def update(self,clients_state_dict):
        ...
        global_grad= []
        for i in range(len(clients_state_dict[0])):
            global_grad.append(torch.zeros_like(clients_state_dict[0][i]))
        for client_state in clients_state_dict:
            global_grad = self.aggregate_model(global_grad,client_state,ratio)
        return global_grad
    def aggregate_model(self,glob, local, ratio):
        for i in range(len(local)):
            glob[i] += local[i]* ratio
        return glob
